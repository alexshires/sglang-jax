...
def test_enable_tokenizer_batch_send_flag_parsing():
    ...
    args = parser.parse_args(
        ["--model-path", "dummy-model", "--enable-tokenizer-batch-send"]
    )
    server_args = ServerArgs.from_cli_args(args)
...
def test_score_v2_adaptive_chunk_args_defaults():
    ...
    server_args = ServerArgs.from_cli_args(args)
    assert (
        server_args.multi_item_score_from_cache_v2_adaptive_chunk_by_token_budget
        is False
    )
...
def test_score_scheduler_controls_parse():
    ...
    assert (
        server_args.score_scheduler_global_microbatch_poll_interval_ms
        == pytest.approx(0.25)
    )
    ...
    assert (
        server_args.score_scheduler_dynamic_items_per_step_short_lane_bias
        == pytest.approx(0.9)
    )
    assert (
        server_args.score_scheduler_dynamic_items_per_step_long_lane_bias
        == pytest.approx(0.6)
    )
...
def test_score_scheduler_controls_reject_negative_window():
    ...
    server_args = ServerArgs.from_cli_args(args)
    with pytest.raises(
        AssertionError, match="score-scheduler-global-microbatch-window-ms"
    ):
        server_args.check_server_args()
...
def test_score_scheduler_controls_reject_non_positive_lane_isolation_burst():
    ...
    server_args = ServerArgs.from_cli_args(args)
    with pytest.raises(
        AssertionError, match="score-scheduler-lane-isolation-short-burst"
    ):
        server_args.check_server_args()
